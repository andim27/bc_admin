...
class TeamController extends BaseController
{
    ...
    public function actionReferrals()
    {
        ...
        foreach ($users as $key => $user) {
            ...
            $result[] = [
                ...
                'lat' => (isset($user->settings->onMapX) && $user->settings->onMapX) ? $user->settings->onMapX : '',
                'lng' => (isset($user->settings->onMapY) && $user->settings->onMapY) ? $user->settings->onMapY : '',
    ...
    public function actionTakeJson()
    {
        ...
        if ($user) {
            ...
            $result = $resultTree ? api\User::buildDiagramData($resultTree, array_keys($resultTree)[0]) : [];
        } else {
            $result = [];
        }
    ...
    public function actionChangeManualRegistration()
    {
        if (Yii::$app->request->isGet) {
            ...
            if ($this->user->settings->manualRegistrationControl == 0 && $manualRegistrationControl == 1 && $this->user->rank < api\User::RANK_8) {
                $result = false;
                $error = THelper::t('team_genealogy_user_not_director');
            } else {
            ...
            return ['success' => $result, 'error' => (isset($error) && $error) ? $error : ''];
        }
    }
}
