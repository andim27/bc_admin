...
class ProfileForm extends Model {
    ...
    public function rules() {
        return
                [
                    [['name', 'surname', 'birthday', 'login', 'email', 'mobile'], 'required', 'message' => THelper::t('required_field')],
                    [['mobile', 'smobile', 'phoneWellness'], 'match', 'pattern' => '/^\+?\d*$/u', 'message' => THelper::t('only_numbers')],
                    ...
                    ['name', 'string', 'message' => THelper::t('only_the_characters')],
                    ['surname', 'string', 'message' => THelper::t('only_the_characters')],
                    ...
                    ['crop_info', 'safe'],
                    ['mobile', 'checkPhone'],
                    ['smobile', 'checkPhoneVipVip'],
                    ['phoneWellness', 'checkPhoneWellness'],
                    ['id', 'string'],
                    ['login', 'checkLogin'],
                    ['email', 'checkEmail']
        ];
    ...
    public function checkPhoneVipVip($attribute, $params)
    {
        $user = api\User::get($this->smobile);

        if ($user && $user->id != $this->id) {
            $this->addError($attribute, THelper::t('this_phone_vip_vip_is_already_in_use'));
        }
    }

    public function checkPhoneWellness($attribute, $params)
    {
        $user = api\User::get($this->phoneWellness);

        if ($user && $user->id != $this->id) {
            $this->addError($attribute, THelper::t('this_phone_wellness_is_already_in_use'));
        }
    }
    ...
    public function checkLogin($attribute, $params)
    {
        ...
        if ($user && $user->id != $this->id) {
            $this->addError($attribute, THelper::t('this_login_is_already_in_use'));
        }
    }
    ...
    public function afterSave($userId) {
        ...
        if ($this->crop_info) {
            ...
            $cropInfo = [
                'dWidth' => $imageSize->getWidth(),
                'dHeight' => $imageSize->getHeight(),
                'x' => 0,
                'y' => 0
            ];
        }
